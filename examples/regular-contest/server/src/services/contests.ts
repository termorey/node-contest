...
export const contestsApi = createApi($contests, {
  addContest: (state, contest: ContestObj) => ({
    ...state,
    list: [...state.list, contest],
  }),
  changeContestStatus: (
    state,
    {
      contestId,
      status,
    }: { contestId: string; status: Partial<ContestStatus> },
  ) => ({
    ...state,
    list: state.list.map((contest) =>
      contestId === contest.id
        ? { ...contest, status: { ...contest.status, ...status } }
        : contest,
    ),
  }),
});
  ...
  async ({
    ...
    contest.addEventListener(ContestEvent.finished, () => {
      console.log(
        "finished",
        id,
        contest.game.engine["regular"].isGameFinished(),
      );
