...
const stepClickedAndApplied = createEvent<{
  contestId: string;
  position: Position;
}>();
sample({
...
const stepClickedToMake = createEvent<{
  contestId: string;
  position: Position;
}>();
sample({
...
export const ContestPage = () => {
  ...
  const handleClickAndApply: (
    contestId: string,
    position: Position,
  ) => () => void = (contestId, position) => () => {
    stepClickedAndApplied({ contestId, position });
  };
  const handleClickToMake: (
    contestId: string,
    position: Position,
  ) => () => void = (contestId, position) => () => {
          ...
          <div className={"grid grid-cols-2 gap-4"}>
            <div className={"flex flex-col justify-start items-stretch gap-2"}>
              ...
              {contest.availablePositions.length > 0 ? (
                <div
                  className={
                    "flex flex-row flex-wrap justify-start items-center gap-2"
                  }
                >
                  {contest.availablePositions.map(([x, y]) => (
                    <Button
                      onClick={handleClickAndApply(contest.id, [x, y])}
                      key={`${x}_${y}`}
                    >{`[${x} ${y}]`}</Button>
                  ))}
                </div>
            ...
            <div className={"flex flex-col justify-start items-stretch gap-2"}>
              ...
              <p>
                <b>{"New tab = new user"}</b>
                <br />
                {"one user = one step per apply"}
              </p>
              <span>{"Available steps:"}</span>
              {contest.availablePositions.length > 0 ? (
                <>
                  ...
                  >
                    {contest.availablePositions.map(([x, y]) => (
                      <Button
                        onClick={handleClickToMake(contest.id, [x, y])}
                        key={`${x}_${y}`}
                      >{`[${x} ${y}]`}</Button>
                    ))}
                  </div>
