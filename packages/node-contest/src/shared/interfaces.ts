...
export type TypedEventTarget<Target extends EventTarget> = {
  new (): Target;
  prototype: Target;
};
...
export interface ChunkInterface {
  checkPositionsEqual: (
    chunk_first: GameChunk,
    chunk_second: GameChunk,
  ) => boolean;
...
export interface DrawInterface {
  ...
  fillChunk: (
    position: { size: Size; position: FieldPosition },
    options: { color: string },
  ) => void;
}
export type Next = (steps: Step[]) => { resolved: Step[]; rejected: Step[] };
export type ExportImageFile = (options: {
  exportPath?: string;
  name?: string;
  format?: string;
}) => Promise<void>;
