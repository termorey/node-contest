...
export class GameCore {
  ...
  readonly engine: {
    [engine: string]: {
      ...
      setStep: <S extends Step>(data: {
        step: S;
      }) => { step: S; updatedChunk: GameChunk | null };
    };
  } = {
    regular: {
      generateGameField: (fieldSize) => {
      ...
      generateGameBank: (bank) => {
        const game = this;
        const condition: (chunk: GameChunk) => boolean = ({ status }) =>
          status.available && !status.checked;
      ...
      setStep: ({ step }) => {
        const game = this;
        const condition: (chunk: GameChunk) => boolean = (chunk) =>
          chunk.status.available && !chunk.step;
  ...
  readonly game: {
    ...
    makeSteps: ({ steps }) => {
      const result = steps.map((step) =>
        this.engine["regular"].setStep({ step }),
      );
      const filteredSteps = result.reduce<FilteredSteps>(
        (prev, { step, updatedChunk }) => {
          if (updatedChunk)
            return {
              ...prev,
              resolved: [...prev.resolved, { step, updatedChunk }],
            };
      ...
      this._contest.dispatchEvent(
        new CustomEvent(ContestEvent.steps, { detail: stepsData }),
      );
      return stepsData;
    },
  };
  ...
  readonly bank: {
    ...
    checkOnPrize: ([x, y]) => {
      return this.gameBank.filter(({ positions }) =>
        positions.some(([posX, posY]) => posX === x && posY === y),
      );
    },
  ...
  readonly chunk: ChunkInterface & {
    getAllByCondition: (
      condition: (chunk: GameChunk) => boolean,
    ) => GameChunk[];
    updateChunkStatus: (
      position: Position,
    ...
    getFieldSize: () => {
      ...
      const counts =
        this._contest.config.fieldsCount ||
        this._contest.defaultConfig.fieldsCount;
      ...
      const height =
        (this._contest.config.fieldSize.height - frameY - gapsY) / partsY;
      const width =
        (this._contest.config.fieldSize.width - frameX - gapsX) / partsX;
    ...
    getPositions: (count, onlyFree) => {
      const chunks = onlyFree
        ? this.gameChunks.filter((c) => c.status.checked)
        : this.gameChunks;
      let selectedChunks: GameChunk[] = [];
      return Array.from({
        length: count > chunks.length ? chunks.length : count,
      }).map(() => {
        const index = this.utils.getRandomInteger({
          from: 0,
          to: chunks.length - 1,
        });
        ...
        while (
          selectedChunks.some((c) => this.chunk.checkPositionsEqual(c, chunk))
        ) {
  ...
  readonly utils = {
    getRandomInteger: ({ from = 0, to = 100 }) =>
