...
export class Snapshot implements SnapshotData {
  ...
  constructor(_contest: Contest, chunks: GameChunk[], lastSteps: Step[]) {
    ...
    this.canvas = createCanvas(
      _contest.config.fieldSize.width,
      _contest.config.fieldSize.height,
    );
  ...
  readonly drawer: DrawInterface = {
    ...
    redraw: async () => {
      ...
      if (this._contest.config.backgroundImage)
        await this.drawer.drawBackgroundImage(
          this._contest.config.backgroundImage,
        );
      ...
      new Promise((resolve) => {
        ...
        image.onload = () => {
          ctx.drawImage(
            ...
            0,
            0,
            this._contest.config.fieldSize.width,
            this._contest.config.fieldSize.height,
          );
          resolve();
        };
        image.src = link;
    ...
    clearField: () => {
      const backgroundColor =
        this._contest.config.backgroundColor ||
        this._contest.defaultConfig.backgroundColor;
    ...
    fillChunk: (
      { size: { height, width }, position: { top, left } },
      { color },
    ) => {
      const ctx = this.drawer.getContext();
      ctx.fillStyle = color;
  ...
  readonly export: SnapshotExport = {
    imageFile: async ({
      ...
      const out = fs.createWriteStream(
        path.join(exportPath, `${name}.${format}`),
      );
      const stream = this.canvas.createPNGStream();
      stream.pipe(out);
      out.on("finish", () =>
        console.log(`The ${name}.${format} file was created.`),
      );
    },
    imageString: async () => {
      await this.drawer.redraw();
