import type {
  ...
  Next,
  SnapshotExport,
  TypedContestEventTarget,
} from "../shared/interfaces";
import { Snapshot } from "./snapshot";
import { GameCore } from "./gameCore";

export class Contest
  extends (EventTarget as TypedContestEventTarget)
  implements ContestInterface
{
  ...
  constructor(payload: { config: Config }) {
    ...
    const size = {
      height:
        this.config.fieldsCount.height || this.defaultConfig.fieldsCount.height,
      width:
        this.config.fieldsCount.width || this.defaultConfig.fieldsCount.width,
    };
  ...
  readonly next: Next = (steps) => {
    ...
    const snapshot = new Snapshot(
      this,
      result.gameSnapshot.gameChunks,
      resolvedSteps,
    );
    this.snapshots = [...this.snapshots, snapshot];
